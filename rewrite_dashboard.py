...
path_colors_map = {
    "المسار الدولي": "#8E44AD"
}
element_colors_map = {}

# Fixed distinct colors for Paths (avoiding standard blues)
AVAILABLE_PATH_COLORS = [
    "#E74C3C", "#2ECC71", "#F1C40F", "#9B59B6", "#E67E22",
    "#1ABC9C", "#34495E", "#D35400", "#C0392B", "#16A085",
    "#8E44AD", "#F39C12", "#27AE60", "#2C3E50", "#7F8C8D"
]
# Fixed distinct colors for Elements
...
def process_stage_data(file_path):
    try:
        ...
        raw_counts = df_unique_national['العنصر'].value_counts().to_dict() if 'العنصر' in df_unique_national.columns else {}
        ...
        for el in unique_stage_elements:
            ...
            # --- MODIFIED: Assign International elements to 'المسار الدولي' ---
            if el.endswith(" - دولي"):
            ...
            # Record colors to ensure they are assigned
...
# To handle assets/Path/Element/image properly
# We look into assets/ and all its subdirectories to find images for elements.
    ...
    for root, dirs, files in os.walk(ASSETS_DIR):
        for file in files:
            if file.endswith(valid_extensions):
                ...
                # url encode parts (especially Arabic characters)
                path_parts = rel_path.split('/')
                safe_path = '/'.join([urllib.parse.quote(p) if p != 'assets' else p for p in path_parts])

                parent_dir = os.path.basename(root)
                # If parent_dir is an element, assign it
                # If parent of parent is an element, assign it (e.g. assets/Path/Element/Director/img.jpg)
...
<body>
    ...
    <div id="main-wrapper">
        <div class="main-card" id="main-card">
                ...
                <div id="international-container" style="position:absolute; bottom:20px; right:20px; z-index: 10;">
                    <button id="international-btn" style="background:#8E44AD; color:white; border:none; padding:10px 20px; font-family:'Tajawal'; font-weight:800; font-size:14px; border-radius:8px; cursor:pointer; box-shadow:0 4px 6px rgba(0,0,0,0.1); transition:transform 0.2s;" onclick="handleRegionClick('International', 'دولي')" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                        🌍 المخرجات الدولية (المسار الدولي)
                    </button>
                </div>
                <div id="tooltip"></div>
            </div>
            ...
            <div id="sidebar" style="flex:1; padding-right:15px; overflow-y:auto; display:flex; flex-direction:column; gap:15px;">
                ...
                <div class="sec-title">📸 التصورات البصرية</div>
                <div id="element-images-view" style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:20px;"></div>
            </div>
...
html_script = """
    <script>
        ...
        const REGION_AR = __REGIONS_MAP_AR__;
        const PATH_COLORS = __PATH_COLORS_JSON__;
        const ELEMENT_COLORS = __ELEMENT_COLORS_JSON__;
        ...
        function renderComparisonDashboard(totalExec, totalTarget, pctTotal) {
            ...
            Object.keys(hierarchy).forEach((pName, index) => {
                ...
                <div style="background:#fff; border-radius:12px; border:1px solid #e2e8f0; margin-bottom:25px; overflow:hidden;">
                    <div class="path-header" style="background:#1e3d59; color:white; padding:15px; cursor:pointer;" onclick="toggleCompPath('${containerId}')">

                        <div style="display:flex; align-items:center;">
                            <span style="font-size:16px; font-weight:800; flex: 1; text-align: right;">📁 ${pName}</span>
                            <span style="font-size:14px; background:rgba(255,255,255,0.2); padding:4px 12px; border-radius:12px; margin-right:auto;">نظرة عامة ▼</span>
                        </div>
                        ...
                        plugins: {
                            ...
                            datalabels: {
                                ...
                                formatter: (value, ctx) => {
                                    ...
                                    // Split long labels or just show percentage
        ...
        function openElementDetails(elName, event) {
            ...
            currentGalleryImages = images; // Images loaded silently for filtering
        }
        ...
        function renderTablePage() {
            ...
            const totalPages = Math.ceil(currentTableRows.length / rowsPerPage);
            if(currentPage < 1) currentPage = 1;
            if(currentPage > totalPages) currentPage = totalPages;

            const startIdx = (currentPage - 1) * rowsPerPage;
            const endIdx = startIdx + rowsPerPage;
            const pageRows = currentTableRows.slice(startIdx, endIdx);
            ...
            pageRows.forEach(r => {
                ...
                const hasImages = currentGalleryImages && currentGalleryImages.some(img => img.caption && img.caption.includes(r.director));
            ...
            // Pagination Controls
            if (totalPages > 1) {
                tableHtml += `<div style="display:flex; justify-content:center; align-items:center; margin-top:15px; gap:10px;">
                    <button onclick="currentPage--; renderTablePage()" ${currentPage === 1 ? 'disabled' : ''} style="padding:5px 10px; cursor:pointer; background:#1e3d59; color:white; border:none; border-radius:4px;">السابق</button>
                    <span style="font-size:12px; font-weight:bold;">صفحة ${currentPage} من ${totalPages}</span>
                    <button onclick="currentPage++; renderTablePage()" ${currentPage === totalPages ? 'disabled' : ''} style="padding:5px 10px; cursor:pointer; background:#1e3d59; color:white; border:none; border-radius:4px;">التالي</button>
                </div>`;
            }
        ...
        function renderOrgEvents(orgName, elName) {
            ...
            currentTableRows = Object.values(matchingEventsMap);
            currentPage = 1;
            renderTablePage();
        }
function renderGallery(images) {
        ...
        function filterGalleryByDirector(directorName) {
            if (!currentGalleryImages || currentGalleryImages.length === 0) {
                alert("لا توجد صور لهذا العنصر حالياً.");
                return;
            }
            const filtered = currentGalleryImages.filter(img => img.caption && img.caption.includes(directorName));
            if (filtered.length > 0) {
                activeLightboxImages = filtered;
                currentGalleryIndex = 0;
                updateLightbox();
                document.getElementById('lightbox-overlay').style.display = 'flex';
            } else {
                alert("لم يتم العثور على مجلد صور مطابق لاسم هذا المخرج.");
            }
        }
        ...
        function handleElementRowClick(elementName, event) {
            ...
            if (selectedElementFilter === elementName) { selectedElementFilter = null; document.getElementById(rowId).classList.remove('active-element-filter'); }
            else { document.querySelectorAll('.element-row').forEach(r => r.classList.remove('active-element-filter')); selectedElementFilter = elementName; document.getElementById(rowId).classList.add('active-element-filter'); }
            refreshMapColors();
        }
